...
seeds_all <- read_excel(
  here("..", "raw-data", "combined_weed_seeds.xlsx"),
  na = c("na", "Na", "NA")
) |>
  clean_names() |>
  mutate(
  ...
  dplyr::select(
    ...
    weed_trt, seed_number, seeds_per_m2_sample
  )

# ---- Plot-season totals normalized by total area sampled --------------

plot_totals <- seeds_all |>
  group_by(site_year, block, plot, weed_trt) |>
  summarise(
...
```{r}
# 1) Summary table: total seed rain by site-year × mowing ---------------
#    (plot-level seasonal totals, normalized per m²)
...
plot_totals |>
  ...
  kable(
    digits  = 1,
    caption = "Weed seed rain (seeds m⁻² season⁻¹) by site-year × mowing treatment (plot-level seasonal totals)"
  ) |>
  kable_styling(
    full_width        = FALSE,
...
#    (plot-level seasonal seed rain per m²)
...
plot_totals |>
  ...
  labs(
    x     = NULL,
    y     = expression(Seed~rain~"(seeds"~m^{-2}~season^{-1}*")"),
    title = "Weed seed rain by mowing treatment across site-years"
  ) +
  theme(
...
## comittee
```{r}
#### Pooled: model testing / selection for seed rain (seeds m⁻² season⁻¹) ----

options(contrasts = c("contr.sum", "contr.poly"))

# Use plot-level seasonal totals (per m²), drop rows with no sampling
seed_totals <- plot_totals |>
  filter(!is.na(seeds_per_m2_season))

# Fixed: mowing × site_year
# Random: site_year and block(site_year)
form_seed_all <- seeds_per_m2_season ~ weed_trt * site_year +
  (1 | site_year) +
  (1 | site_year:block)
...
aic_seed <- tibble(
  model = c(
    ...
    "SEED: NB2",
    "SEED: Tweedie + ZI",
    "SEED: Tweedie + ZI + disp(site_year)"
  ),
  AIC = c(
    safe_aic(m_seed_tw),
    safe_aic(m_seed_nb),
    safe_aic(m_seed_tw_zi),
    safe_aic(m_seed_tw_disp)
  )
)
...
kable(
  aic_seed,
  digits  = 1,
  caption = "Pooled seed rain (seeds m⁻² season⁻¹): candidate distributional families"
) |>
...
best_seed   <- m_seed_tw
best_seed_n <- "SEED: Tweedie"
best_aic    <- safe_aic(m_seed_tw)

seed_cands <- list(
  "SEED: NB2"                            = m_seed_nb,
  "SEED: Tweedie + ZI"                   = m_seed_tw_zi,
  "SEED: Tweedie + ZI + disp(site_year)" = m_seed_tw_disp
)

for (nm in names(seed_cands)) {
  this_aic <- safe_aic(seed_cands[[nm]])
  
  # Only consider this candidate if its AIC is finite
  if (is.finite(this_aic)) {
    
    # If the current "best" AIC is not finite, accept first finite candidate
    if (!is.finite(best_aic) || (this_aic + 2 < best_aic)) {
      best_seed   <- seed_cands[[nm]]
      best_seed_n <- nm
      best_aic    <- this_aic
    }
  }
...
if (!is.finite(best_aic)) {
  stop("All candidate seed models failed (AIC is NA). Check glmmTMB fits.")
}

cat("Selected family/structure for seed rain:", best_seed_n, "\n")

## 3) Final model: interaction in chosen family -------------------------

# best_seed already has:
#   seeds_per_m2_season ~ weed_trt * site_year
# plus the chosen random/distributional structure.

seed_glmm <- best_seed

cat(
  "\nSelected model for seed rain (used in all downstream emmeans/plots):\n  ",
  "seeds_per_m2_season ~ weed_trt * site_year",
  "\n  Family/structure:", best_seed_n, "\n"
)

## 4) Diagnostics + Type-III tests for the chosen model -----------------

set.seed(123)
res_seed <- DHARMa::simulateResiduals(seed_glmm)
plot(res_seed)
DHARMa::testDispersion(seed_glmm)
DHARMa::testZeroInflation(seed_glmm)

car::Anova(seed_glmm, type = 3)

```

```{r}
### Seed rain (seeds m⁻² season⁻¹) with Fisher's LSD CLDs (by mowing)
### Using Tweedie GLMM with weed_trt * site_year

# Estimated marginal means for weed_trt on the response scale (seeds/m²/season)
emm_seed <- emmeans(
  seed_glmm,
  ~ weed_trt,
...
# Tidy emmeans for reporting
emm_seed_df <- tidy_emm(
  emm_seed,
  trt_var    = "weed_trt",
  ref_levels = mow_levels
) |>
  dplyr::rename(
    emmean = response    # rename mean on response scale
  ) |>
  dplyr::select(
    weed_trt, emmean, SE, ci_low, ci_high
  ) |>
  dplyr::mutate(
    dplyr::across(c(emmean, SE, ci_low, ci_high), ~ round(.x, 1))
  )

# Compact letter display (Fisher's LSD, no adjustment; "a" = highest seed rain)
cld_seed <- cld(
  emm_seed,
  adjust   = "none",
  Letters  = letters,
  sort     = TRUE,
  reversed = TRUE   # so "a" = highest seed rain group(s)
) |>
  as_tibble() |>
  mutate(
    weed_trt = factor(weed_trt, levels = mow_levels),
...
emm_seed_df |>
  left_join(cld_seed, by = "weed_trt") |>
  kable(
    caption   = "Estimated weed seed rain (seeds m⁻² season⁻¹) by mowing treatment (pooled over site-years) with 95% CI and Fisher's LSD group letters from a Tweedie GLMM.",
    col.names = c("Mowing treatment", "Mean", "SE", "Lower CI", "Upper CI", "Group")
  ) |>
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
...
```
```{r}
#### Figure: Seed rain by mowing treatment (raw means ± SE, model CLDs)

# 1) Raw plot-level means and SE by mowing ------------------------------
raw_seed_summary <- plot_totals |>
  filter(!is.na(seeds_per_m2_season)) |>
  group_by(weed_trt) |>
  summarise(
    n    = n(),
...
# 2) Use the CLDs we already computed (cld_seed) ------------------------
plot_df_seed <- raw_seed_summary |>
  left_join(cld_seed, by = "weed_trt")

# 3) Plot ---------------------------------------------------------------
ggplot(plot_df_seed, aes(x = weed_trt, y = mean, fill = weed_trt)) +
  ...
  labs(
    x       = NULL,
    y = expression(Seed~rain~"(seeds"~m^{-2}*")"),
...
# 4) Save figure --------------------------------------------------------
ggsave(
  filename = here("figures", "fig_seed_rain_mowing_pooled_raw.png"),
  width    = 7.5,
  height   = 5.5,
  dpi      = 300
)
