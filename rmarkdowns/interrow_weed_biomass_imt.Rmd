...
clean_combined <- clean_names(combined_raw) |>  
  ...
  mutate(across(c(year, location, site_year, mowing, block, plot, weeds, ), as.factor),interrow_weed_biomass = as.numeric(as.character(interrow_weed_biomass)))  

#select and convert data for wbm analysis
interrow_weed_biomass_clean <-clean_combined |>             
  filter(weeds %in% c("SW", "M")) |>  # Keep rows where SW and M are microplots
  filter(location %in% c("WI", "VT", "FH", "CU")) |>  # Keep rows where WI, VT, FH, and CU and M are locations
...
model_tweedie_log <- glmmTMB(interrow_weed_biomass_lbs_ac ~ mowing*weeds + (1|site_year) + (1|site_year:block)+ (1|site_year:block:mowing), 
...
```{r}
cld_location_fisher <-cld(emmeans(model_tweedie_log, ~  weeds, type = "response"), Letters = letters,adjust = "none", sort = TRUE, reversed=TRUE)
cld_location_fisher
...
ggsave("interrow_weed_biomass_mowing_lbsacre.png", width = 10, height = 8, dpi = 300)
```