...
```{r}
cld_mowing_fisher <-cld(emmeans(random_eco, ~  mowing , type = "response"), Letters = letters, adjust = "none",sort = TRUE, reversed=TRUE)
cld_mowing_fisher
```
...
```{r}
cld_weeds_fisher <-cld(emmeans(random_eco, ~  weeds , type = "response"), Letters = letters, adjust = "none",sort = TRUE, reversed=TRUE)
cld_weeds_fisher
```
...
```{r}
cld_weeds_location_fisher_fh <-cld(emmeans(random_fh, ~  weeds|location , type = "response"), Letters = letters, adjust = "none",sort = TRUE, reversed=TRUE)
cld_weeds_location_fisher_fh
```
#### Mowing by location (not significant)

```{r}
cld_mowing_location_fisher_fh <-cld(emmeans(random_fh, ~  mowing|location , type = "response"), Letters = letters, adjust = "none",sort = TRUE, reversed=TRUE)
cld_mowing_location_fisher_fh
```
...
bean_yield_clean |> 
  ...
  ggplot(aes(x = factor(mowing, levels = c("NWC", "EWC", "LWC", "AWC")), y = bean_yield_adj_bu_acre_fh, fill = mowing)) +
...
#geom_text(aes(label = trimws(.group), y = response + (SE + 30)), size = 7) +
  labs(
    x = "",
       y = expression(paste("Dry bean yield (", bu~a^{-1}, " at 14% moisture)")))+
