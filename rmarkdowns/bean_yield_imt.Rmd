...
plot_df_yield_ny_model_bu <- plot_df_yield_ny_model |>
  dplyr::mutate(
    mean_bu = mean * kg_ha_to_bu_ac,
    SE_bu   = SE   * kg_ha_to_bu_ac,
    ymin_bu = pmax(mean_bu - SE_bu, 0),
...
fig_ny_yield_total_model_bu <- ggplot(
  plot_df_yield_ny_model_bu,
  aes(x = weed_trt, y = mean_bu, fill = weed_trt)
) +
  geom_col(width = 0.7, color = "black") +
  geom_errorbar(aes(ymin = ymin_bu, ymax = ymax_bu), width = 0.14) +
  ...
  labs(
    ...
    y       = expression(Bean~yield~"(bu"~ac^{-1}*" at 14% moisture)"),
    #title   = "New York (Cornell + Farm Hub): bean yield by mowing treatment",
    #caption = "Bars show LMM-predicted marginal means (bu ac\u207b\u00b9, adjusted to 14% moisture) \u00b1 SE."
  ) +
  theme_classic(base_size = 18) +
  theme(
    axis.text.x  = element_text(lineheight = 0.95, margin = margin(t = 8)),
...
fig_ny_yield_total_model_bu

ggsave(
  filename = file.path(fig_dir_yield_ny, "fig_ny_yield_total_model_bu_ac.png"),
  plot     = fig_ny_yield_total_model_bu,
  width    = 9,
  height   = 5.5,
  dpi      = 300
)
...
raw_ny_yield_fig_bu <- raw_ny_yield_fig |>
  dplyr::mutate(
    mean_bu = mean * kg_ha_to_bu_ac,
    se_bu   = se   * kg_ha_to_bu_ac,
    ymin_bu = pmax(mean_bu - se_bu, 0),
...
fig_ny_yield_total_raw_bu <- ggplot(
  raw_ny_yield_fig_bu,
  aes(x = weed_trt, y = mean_bu, fill = weed_trt)
) +
  geom_col(width = 0.7, color = "black") +
  ...
  labs(
    x       = NULL,
    y       = expression(Bean~yield~"(bu"~ac^{-1}*")"),
    title   = "New York (Cornell + Farm Hub): bean yield by mowing treatment",
    caption = "Bars show raw treatment means (bu ac\u207b\u00b9, adjusted to 14% moisture) \u00b1 SE."
  ) +
  theme_classic(base_size = 18) +
...
fig_ny_yield_total_raw_bu

ggsave(
  filename = file.path(fig_dir_yield_ny, "fig_ny_yield_total_raw_bu_ac.png"),
  plot     = fig_ny_yield_total_raw_bu,
  width    = 9,
  height   = 5.5,
  dpi      = 300
)
...
# Weed-free benchmark (NY-only) -----------------------------------------
weedfree_mean_kg_ha <- bean_yield_sys_ny |>
  dplyr::filter(weeds3 == "Weed-free") |>
  dplyr::summarise(mu = mean(yield_kg_ha_adj14, na.rm = TRUE)) |>
  dplyr::pull(mu)

weedfree_mean_bu_ac <- weedfree_mean_kg_ha * kg_ha_to_bu_ac
...
```{r}
# ----------------------------------------------------------------------
# 1b) MODEL-PREDICTED MEANS (bu/ac) + weed-free mean reference line
# ----------------------------------------------------------------------
...
weedfree_mean_bu_ac <- weedfree_mean_kg_ha * kg_ha_to_bu_ac
wf_label <- sprintf("%.1f bu/ac", weedfree_mean_bu_ac)

# Convert model-predicted mowing means to bu/ac --------------------------
plot_df_yield_ny_model_bu <- plot_df_yield_ny_model |>
  dplyr::mutate(
    mean_bu = mean * kg_ha_to_bu_ac,
    SE_bu   = SE   * kg_ha_to_bu_ac,
...
fig_ny_yield_total_model_bu <- ggplot(
  ...
  geom_col(width = 0.7, color = "black") +
  geom_errorbar(aes(ymin = ymin_bu, ymax = ymax_bu), width = 0.14) +
  geom_hline(yintercept = weedfree_mean_bu_ac, linetype = "dashed", linewidth = 0.8) +
  annotate(
    "text",
    x = Inf, y = weedfree_mean_bu_ac,
    label = wf_label,              # "31.6 bu/ac"
  ...
  labs(
    x = NULL,
    y = "Bean yield (bu/ac at 14% moisture)"
  ) +
  theme_classic(base_size = 18) +
  theme(
    axis.text.x  = element_text(lineheight = 0.95, margin = margin(t = 8)),
