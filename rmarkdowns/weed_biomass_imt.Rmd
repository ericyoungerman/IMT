...
```{r}
## Sanity check: does the weed_trt × weeds interaction matter? -----------

# 1) Fit additive + interaction models in the *same* family/ZI/disp -----
# (assumes ny_best is your chosen family/ZI/disp model from above)

wbm_ny_int <- ny_best  # weed_trt * weeds + site_year + random effects
wbm_ny_add <- update(wbm_ny_int, . ~ weed_trt + weeds + site_year)

# 2) LRT (nested model comparison) --------------------------------------
lrt_ny <- anova(wbm_ny_add, wbm_ny_int)
print(lrt_ny)

# 3) AIC check ----------------------------------------------------------
AIC_add <- AIC(wbm_ny_add)
AIC_int <- AIC(wbm_ny_int)
cat("\nAIC(add) =", AIC_add,
    "\nAIC(int) =", AIC_int,
    "\nΔAIC (add - int) =", AIC_add - AIC_int, "\n")

# 4) Wald test for the interaction term --------------------------------
# Note: glmmTMB doesn't always give Type III interaction p-values directly.
# This gives a joint Wald test for the interaction block.
car::Anova(wbm_ny_int, type = 3)

lme4::nobars(formula(wbm_ny_add))  # fixed
lme4::findbars(formula(wbm_ny_add))# random
lme4::findbars(formula(wbm_ny_int))
```


```{r}
## Inspect interaction patterns (sensitivity) ----------------------------

# 1) EMMs by weed_trt within weeds level (interaction model) -------------
emm_int <- emmeans::emmeans(
  wbm_ny_int,
  ~ weed_trt | weeds,
  type = "response"
)

# Get CIs explicitly (GLMMs often return asymp.LCL / asymp.UCL)
emm_int_ci <- summary(emm_int, infer = c(TRUE, TRUE))
emm_int_df <- as.data.frame(emm_int_ci)

print(emm_int_df)

# 2) Pairwise within each weeds level (LSD-style: no multiplicity adj) ---
pairs_int <- emmeans::contrast(
  emm_int,
  method = "pairwise",
  by = "weeds",
  adjust = "none"
)
print(as.data.frame(pairs_int))

# 3) Simple interaction plot (model-predicted means + CI bars) ----------
# Uses asymp.LCL/asymp.UCL when present; falls back to SE if not.
has_ci <- all(c("asymp.LCL", "asymp.UCL") %in% names(emm_int_df))

p_int_plot <- ggplot(emm_int_df, aes(x = weed_trt, y = response, group = weeds)) +
  geom_point(position = position_dodge(width = 0.2)) +
  geom_line(position = position_dodge(width = 0.2)) +
  facet_wrap(~ weeds) +
  theme_classic(base_size = 14) +
  labs(x = NULL, y = "Predicted weed biomass (kg ha^-1)")

if (has_ci) {
  p_int_plot <- p_int_plot +
    geom_errorbar(
  ...
  p_int_plot <- p_int_plot +
    geom_errorbar(
      aes(ymin = pmax(response - SE, 0), ymax = response + SE),
      width = 0.12,
      position = position_dodge(width = 0.2)
    )
}
...
emm_add_by_weeds <- emmeans::emmeans(wbm_ny_add, ~ weed_trt | weeds, type = "response")
summary(emm_add_by_weeds)
pairs(emm_add_by_weeds, by = "weeds", adjust = "none")
```
...
# Conversions -----------------------------------------------------------
kg_ha_to_lb_ac    <- 0.892179
lb_per_bu_beans   <- 60                 # dry beans: 60 lb per bushel
kg_ha_to_bu_ac    <- kg_ha_to_lb_ac / lb_per_bu_beans  # kg/ha -> bu/ac
...
# 1) MODEL-PREDICTED MEANS (kg ha^-1)
# ----------------------------------------------------------------------
# Uses: emm_trt_wbm_ny_df (must contain model_mean and SE) and cld_wbm_ny (trt_CLD)
...
fig_ny_wbm_total_model_bu <- ggplot(
  ...
  geom_text(
    aes(y = ymax_bu * 1.08, label = trt_CLD),
    vjust    = 0,
    fontface = "bold",
    size     = 6
  ) +
  ...
  labs(
    ...
    title   = NULL,
    #caption = "Bars show GLMM-predicted marginal means (bu ac\u207b\u00b9) \u00b1 SE; letters denote Fisher\u2019s LSD groups (\u03b1 = 0.05)."
  ) +
  theme_classic(base_size = 18) +
  theme(
    axis.text.x  = element_text(lineheight = 0.95, margin = margin(t = 8)),
...
ggsave(
  filename = file.path(fig_dir_ny_total, "fig_ny_weed_biomass_total_model_bu_ac.png"),
...
fig_ny_wbm_total_raw_bu <- ggplot(
  ...
  labs(
    ...
    y       = expression(Weed~biomass~"(bu"~ac^{-1}*")"),
    title   = "New York (Cornell + Farm Hub): total weed biomass by mowing",
    caption = "Bars show raw treatment means (bu ac\u207b\u00b9) \u00b1 SE; letters reuse GLMM-based Fisher\u2019s LSD groups (\u03b1 = 0.05)."
  ) +
  theme_classic(base_size = 18) +
  theme(
    axis.text.x  = element_text(lineheight = 0.95, margin = margin(t = 8)),
...
ggsave(
  filename = file.path(fig_dir_ny_total, "fig_ny_weed_biomass_total_raw_bu_ac.png"),
