...
model_tweedie_log <- glmmTMB(weed_biomass_lbs_ac ~ mowing*weeds*location + (1|location:block)+ (1|location:block:mowing), 
...
```{r}
cld_weeds_fisher <-cld(emmeans(model_tweedie_log, ~  location, type = "response"), Letters = letters,adjust = "none", sort = TRUE, reversed=TRUE)
cld_weeds_fisher
...
```{r}
cld_weeds_fisher_fh <-cld(emmeans(model_tweedie_log_fh, ~  location, type = "response"), Letters = letters,adjust = "none", sort = TRUE, reversed=TRUE)
cld_weeds_fisher_fh


```
weed level, location, mowing
